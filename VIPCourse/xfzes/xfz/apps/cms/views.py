...
@staff_member_required(login_url='index')#不是公司的员工无感访问这个视图函数，如果访问了会指定到这个url下面
def index(request):
    return render(request,'cms/index.html')

class NewsListView(View):
    def get(self,request):
        page = int(request.GET.get('p',1))#如果p为空就默认为传1
        ...
        context = {
            ...
            # 因为会用到category的name和author的那么，所以查询的时候使用select_related()
            'newses': page_obj.object_list,#当前这一页的对象
            'page_obj':page_obj,
            'paginator':paginator,
            'start':start,
            'end':end,
...
@require_GET
def qntoken(request):
    # access_key = 'eQhVp2xRuMTichslHx4dnlIqbAq5IH0QFmDJqx-O'
    access_key = settings.QINIU_ACCESS_KEY
    # secret_key = 'Mw-_7FQyn3O382OdRQ9UYRLiHYwwRTsmll3OXBwB'
    secret_key = settings.QINIU_SECRET_KEY
    # bucket = 'musk_118' #空间名称
    bucket = settings.QINIU_BUCKET_NAME #空间名称
    q = qiniu.Auth(access_key,secret_key)#授权信息
    token = q.upload_token(bucket) #生成token，把bucket传过去,返回给js，js拿到之后跟七牛云进行交互
